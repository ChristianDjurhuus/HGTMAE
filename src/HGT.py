...
class HGTLayer(nn.Module):
    def __init__(self, in_dim, out_dim, n_heads, node_type_list, edge_type_list, edge_type_mapping, last_residual = True, last_norm = True, device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')):
...
class HGT(nn.Module):
    def __init__(self,
        ...
        if n_layers == 1:
            self.layers.append(HGTLayer(in_dim, out_dim, n_heads, node_type_list, edge_type_list, edge_type_mapping, last_residual = False))
        #self.layers.append(HGTLayer(in_dim, hidden_dim, n_heads, node_type_list, edge_type_list, edge_type_mapping))
        else: 
            for _ in range(n_layers-1):
                self.layers.append(HGTLayer(hidden_dim, hidden_dim, n_heads, node_type_list, edge_type_list, edge_type_mapping))
            # Last layer
            self.layers.append(HGTLayer(hidden_dim, out_dim, n_heads, node_type_list, edge_type_list, edge_type_mapping))
